...
from .models import SampleLeaderboard
...
#class StudentDashboardPageView(LoginRequiredMixin, TemplateView):
...
class StudentUserPageView(TemplateView):
    template_name = "user.html"

class StudentLeaderboardPageView(TemplateView):
...
def get_all_students(request):
    try:
        data={}
        students = Users.objects.filter(role='Student').values("id", "first_name", "last_name", "email", "role")
        data["status"] = "success"
        data["students"] = list(students)
        return HttpResponse(json.dumps(data), status=200)
    except Exception as e:
        print("[EXCEPTION][get_all_students] ::: {}".format(e))
        data["status"] = "failure"
        data["reason"] = "Because you made a mistake"
        return HttpResponse(json.dumps(data), status=500)
...
def get_all_users(request):
    try:
        data={}
        students = Users.objects.all().values("id", "first_name", "last_name", "email", "role")
        data["status"] = "success"
        data["students"] = list(students)
        return HttpResponse(json.dumps(data), status=200)
    except Exception as e:
        print("[EXCEPTION][get_all_users] ::: {}".format(e))
        data["status"] = "failure"
        data["reason"] = "Because you made a mistake"
        return HttpResponse(json.dumps(data), status=500)
...
def get_leaderboard(request):
    try:
        data={}
        leaderboard = SampleLeaderboard.objects.all().values("position", "team", "matches_played", "win_percentage", "win_streak", "win_loss")
        data["status"] = "success"
        data["leaderboard"] = list(leaderboard)
        return HttpResponse(json.dumps(data), status=200)
    except Exception as e:
        print("[EXCEPTION][get_leaderboard] ::: {}".format(e))
        data["status"] = "failure"
        data["reason"] = "Because you made a mistake"
        return HttpResponse(json.dumps(data), status=500)
...
def get_name(request):
    #print("first thing")
    req_data = json.loads(request.body)
    #print("please show this")
    try:
        error = None
        #print("aaaa")
        data = {}

        name = Users.objects.filer(role=role, email=email).values("first_name", "last_name")
        ...
        print("[EXCEPTION][get_name] ::: {}".format(e))
        data["status"] = "failure"
        data["reason"] = "Because you made a mistake"
        return HttpResponse(json.dumps(data), status=500)